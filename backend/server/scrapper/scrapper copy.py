...
class Selenium:
    ...
    def __fetch_sizes(self, ls):
        ...
        for i in ls:
            try:
                ...
                a = [x.text for x in select.options]
                ...
                #     # Extract the UK size using string manipulation or regular expressions
                #     # Here, assuming the UK size is always the part before the first space
                #     # yield option_text
                #     uk_size = option_text.split(' ', 1)[0]
    ...
    def get_available_sizes(self):
        # with open('available_variation_option', 'a+') as f:
        # self.close_modals(url)

        self.__login()
        with open(f"product.json", "r") as file:
            f = json.load(file)
            sku_ls = [i['fields']['data']['sku'] for i in f[:1]]
            # print(sku_ls[0])
            data_generator = self.__fetch_sizes(sku_ls)
            for entry in enumerate(data_generator):
                print('1')
                print(entry)
