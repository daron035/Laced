...
export default function Footer() {
  ...
  const [preferences, setPreferences] = useState({country: null, currency: null});
  useEffect(() => {
    if (isSuccess && data) {
      setPreferences({ ...preferences, country: data.country_name, currency: data.currency_iso})
    }
  }, [isSuccess, data]);
        ...
        <div className="h-full px-2">
          <div className="flex items-center justify-center h-full">
            ...
            >
              ...
              <span>лангуаге</span>
              <span className="px-1 text-sm font-bold">|</span>
              <span>{isSuccess && `${data.currency_symbol} ${data.currency_iso}`}</span>
            </button>
